...
from PCB.PCB_model import PCB,PCB_test
import pdb
...
class Config(object):
  def __init__(self):
    ...
    parser.add_argument('--PCB', type=str2bool, default=True )
    parser.add_argument('--resize_h_w', type=eval, default=(256, 128))
    parser.add_argument('--PCBresize_h_w', type=eval, default=(384, 129))
    ...
    if self.pcb == False:
      ...
      self.resize_h_w = args.PCBresize_h_w
...
class ExtractFeature(object):
  ...
  def __call__(self, ims):
    ...
    feat = self.model(ims)
    fnorm = torch.norm(feat, p=2, dim=1, keepdim=True) * np.sqrt(6)
...
def main():
  ...
  optimizer_ft = optim.SGD([
             ...
             {'params': model.classifier0.parameters(), 'lr': 0.05},
             {'params': model.classifier1.parameters(), 'lr': 0.05},
             {'params': model.classifier2.parameters(), 'lr': 0.05},
             {'params': model.classifier3.parameters(), 'lr': 0.05},
             {'params': model.classifier4.parameters(), 'lr': 0.05},
             {'params': model.classifier5.parameters(), 'lr': 0.05},
  ...
  def test(load_model_weight=False):
    ...
    for test_set, name in zip(test_sets, test_set_names):
      ...
      test_set.set_feat_func(ExtractFeature(test_model, TVT))
      print('\n=========> Test on dataset: {} <=========\n'.format(name))
      test_set.eval(
  ...
  for ep in range(start_ep, cfg.total_epochs):
    ...
    while not epoch_done:
      ...
      loss = criterion(part[0], labels_t)
      for i in range(num_part-1):
         loss += criterion(part[i+1], labels_t)
