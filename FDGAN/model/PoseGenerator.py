...
def get_current_visuals(origin,target,fake,pose):
    origin = util.tensor2im(origin)
    target = util.tensor2im(target)
    trans_picture = util.tensor2im(fake)
    poses = pose.sum(1)
    poses[poses>1]=1
    poses = util.tensor2im(torch.unsqueeze(poses,1))
    return OrderedDict([('input', origin), ('posemap', poses),('fake', trans_picture),('target', target)])
